...
Bộ tag W6 được giữ nhỏ và nhất quán. Điểm chính là tránh các biến thể không đồng nhất như `taskio`, `Taskio` hoặc tên ứng dụng cũ. Giá trị tag application được dùng trong tuần này là `taskIO`.

| Tag Key | Value |
|---|---|
| Owner | `dinhdanhnam1@gmail.com` |
| Environment | `dev` |
| CostCenter | `G11` |
| Application | `taskIO` |
...
Các baseline resources đã được tag gồm VPC, Internet Gateway, public subnet, route table, security groups, EC2 instances, Systems Manager Parameter Store, Application Load Balancer, target group và listener. Trong workshop environment, compliance được verify thủ công qua AWS Resource Groups Tag Editor. Nếu đưa sang production, tagging nên được enforce bằng Infrastructure as Code, AWS Config rules, SCP policies và periodic tagging audits.

### Cost Allocation Tags

Nhóm đã thử activate custom cost allocation tags trong AWS Billing Console. Tuy nhiên workshop account là AWS Organizations member account, nên billing permissions bị giới hạn ở payer account level. Vì vậy nhóm không thể activate custom cost allocation tags trực tiếp từ sandbox account.
...
AWS Cost Explorer được dùng để review service-level spend và baseline spending trend. Việc group cost theo tag chưa khả dụng vì member account không có quyền activate custom cost allocation tags. AWS Budgets được cấu hình với monthly threshold `150 USD` và notification gửi về owner email để nhóm phát hiện sớm nếu spending bất thường.

| Setting | Value |
|---|---|
| Budget Name | `TaskIO-W6-Budget` |
...
*Figure 9. AWS Budget configuration cho TaskIO-W6-Budget với threshold 150 USD.*
...
Lưu ý: resource tag của AWS stack dùng `Application=taskIO`, nhưng custom metric dimension trong CloudWatch hiện là `Application=TaskIO` theo code Lambda đã deploy. Vì vậy khi add custom metrics vào dashboard, cần chọn dimension `Application=TaskIO`.
